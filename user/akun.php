...
$sql = "SELECT nama, email, password_hash FROM users WHERE id = ?";
...
if ($result->num_rows > 0) {
  ...
  header("Location: error_page.php"); // Redirect to an error page or handle it differently
  exit();
}

// Inisialisasi variabel statement
$stmtUpdate = null;
...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
  ...
  if (!empty($currentPassword) || !empty($newPassword)) {
    ...
    if (!password_verify($currentPassword, $user['password_hash'])) {
      $_SESSION['error'] = "Sandi lama salah.";
    } elseif ($newPassword !== $confirmNewPassword) {
      $_SESSION['error'] = "Sandi baru tidak sama.";
    } else {
      $newPasswordHash = password_hash($newPassword, PASSWORD_DEFAULT);
      $sqlUpdate = "UPDATE users SET nama = ?, email = ?, password_hash = ? WHERE id = ?";
      $stmtUpdate = $conn->prepare($sqlUpdate);
      if ($stmtUpdate) {
        $stmtUpdate->bind_param('sssi', $nama, $email, $newPasswordHash, $user_id);
      } else {
        $_SESSION['error'] = "Error preparing statement for updating password.";
      }
    }
  } else {
    $sqlUpdate = "UPDATE users SET nama = ?, email = ? WHERE id = ?";
    $stmtUpdate = $conn->prepare($sqlUpdate);
    if ($stmtUpdate) {
      $stmtUpdate->bind_param('ssi', $nama, $email, $user_id);
    } else {
      $_SESSION['error'] = "Error preparing statement for updating profile.";
    }
  }
  ...
  if ($stmtUpdate) {
    if ($stmtUpdate->execute()) {
      ...
      $_SESSION['error'] = "Error: " . $stmtUpdate->error;
    }
    $stmtUpdate->close();
  }

  header("Location: akun.php"); // Redirect to the same page to display the message
  exit();
}
      ...
      <div class="layout-page">
        ...
        <div class="content-wrapper">
          ...
          <div class="container-xxl flex-grow-1 container-p-y">
            <h4 class="fw-bold py-3 mb-4"><span class="text-muted fw-light">Beranda /</span> Akun</h4>
              ...
              <div class="col-md-12">
                    ...
                    <form id="formAccountSettings" method="POST">
                      <div class="row">
                        <div class="mb-3 col-md-6">
                          ...
                          <input class="form-control" type="text" id="nama" name="nama" value="<?php echo htmlspecialchars($user['nama']); ?>" autofocus />
                        </div>
                        <div class="mb-3 col-md-6">
                          <label for="email" class="form-label">Surel</label>
                          <input class="form-control" type="email" id="email" name="email" value="<?php echo htmlspecialchars($user['email']); ?>" placeholder="john.doe@example.com" readonly />
                        </div>
                      </div>
                      <div class="row">
                        <div class="mb-3 col-md-6">
                          <label for="currentPassword" class="form-label">Sandi Lama</label>
                          <input class="form-control" type="password" id="currentPassword" name="currentPassword" placeholder="(Kosongkan jika tidak merubah sandi)" />
                        </div>
                        <div class="mb-3 col-md-6">
                          <label for="newPassword" class="form-label">Sandi Baru</label>
                          <input class="form-control" type="password" id="newPassword" name="newPassword" placeholder="(Kosongkan jika tidak merubah sandi)" />
                        </div>
                        <div class="mb-3 col-md-6">
                          <label for="confirmNewPassword" class="form-label">Konfirmasi Sandi Baru</label>
                          <input class="form-control" type="password" id="confirmNewPassword" name="confirmNewPassword" placeholder="(Kosongkan jika tidak merubah sandi)" />
                        </div>
                      </div>
                      <div class="mt-2">
                ...
                <div class="card">
                  ...
                  <div class="card-body">
                    ...
                    <form id="formAccountDeactivation" method="POST" action="hapusakun.php">
